...
class Students extends CI_Controller {
	...
	public function index() {
		...
		if($this->session->userdata('logged_in') == FALSE){
			...
			$permit = array('Admin', 'HOD', 'Bursary', 'Library', 'Sports', 'Dean');
		...
		if($ebs_user_role == 'HOD') {
			$dp_id = $this->Crud->read_field('id', $ebs_user_id, 'eb_user', 'dept_id');
			$allrec = $this->Crud->read2('role', 'Student', 'dept_id', $dp_id, 'eb_user');
		} else {

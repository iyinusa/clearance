...
<?php
    $log_user_id = $this->session->userdata('ebs_id');
    $log_user_role = $this->session->userdata('ebs_user_role');
	...
	if($page_active == 'clearance'){$clearance_act='active';}else{$clearance_act='';}
?>
<!DOCTYPE html>
            ...
            <div class="sidebar-content">
                <div class="side-header side-content bg-white">
                    <a class="h5 text-white" href="<?php echo base_url('profile'); ?>"> 
                        ...
                        <span class="h4 font-w600 sidebar-mini-hide text-black">Clearance</span> 
                    </a> 
              	</div>
                
                <div class="side-content">
                    <ul class="nav-main">
                        <?php $setting_access = array('Admin', 'HOD', 'Bursary', 'Library', 'Sports', 'Dean'); ?>
                        <?php if(in_array($log_user_role, $setting_access)){ ?>
                        ...
                        <li class="<?php if($page_active=='programme' || $page_active=='session' || $page_active=='level' || $page_active=='school' || $page_active=='department' || $page_active=='student'){echo 'open';} ?>"> 
                            ...
                            <ul>
                                ...
                                <li> <a href="<?php echo base_url('accounts'); ?>" class="<?php echo $account_act; ?>">Accounts</a> </li>
                                <?php } ?>
                        ...
                        <!-- <li class="nav-main-heading"><span class="sidebar-mini-hide">Clearance</span></li> -->
                        <li> 
                        	<a href="<?php echo base_url('clearance'); ?>" class="<?php echo $clearance_act; ?>"><i class="si si-book-open"></i><span class="sidebar-mini-hide">Clearance</span></a> 
                     	</li>

                        <li class="nav-main-heading"><span class="sidebar-mini-hide">Notification</span></li>

                        <?php
                            $notify = $this->Crud->read_single('user_id', $log_user_id, 'eb_notify');
                            if(!empty($notify)) {
                                $notify_list = '';
                                foreach($notify as $n) {
                                    $clear_id = $n->clearance_id;
                                    $details = $n->details;
                                    $reg_date = $n->reg_date;
                                    $timestamp = timespan(strtotime($reg_date), time()) . ' ago';
                                    if(strlen($details > 30)) {$short_details = substr($details, 0, 30).'...';} else {$short_details = $details;}

                                    $notify_list .= '
                                        <a href="'.base_url('clearance/form/'.$clear_id).'" target="_blank" style="color:#ddd; display:block; padding-bottom:10px; border-bottom:1px solid #111;">'.$short_details.'<br/><small class="text-muted">'.$timestamp.'</small></a>
                                    ';
                                }
    ...
    <header id="header-navbar" class="content-mini content-mini-full">
        ...
        <ul class="nav-header pull-left">
            ...
            <li class="text-muted small">
            	<b class="text-danger">Laspotech Clearance</b><br />
